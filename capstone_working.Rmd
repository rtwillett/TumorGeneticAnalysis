...
```{r}
#CODE BLOCK IS NON-FUNCTIONAL

cancer_sample <- cancer_join %>% group_by(Class) %>% 
    summarise(
      count = n(),
  
  for (i in 1:ncol(small_gset)) {
...
```{r echo = FALSE}
#Used the code provided by mentor Amit 
remDr <- remoteDriver(remoteServerAddr = "localhost"
                      , port = 4445L
                      , browserName = "firefox"
)

remDr$open() # Throws error that resContent[["status"]]: subscript out of bounds

remDr$navigate("https://www.ncbi.nlm.nih.gov/gene/15")


elem1 <- remDr$findElement(using = "css", value = "#ui-ncbigrid-23 tr:nth-child(5) td:nth-child(1) a") #get big table in text string
elemtxt1 <- elem$getElementAttribute("innerHTML")[[1]] #get us the HTML

elem2 <- remDr$findElement(using = "css", value = "#ui-ncbigrid-27 tr~ tr+ tr a") #get big table in text string
#elem$highlightElement() #just for interactive use in browser. not necessary. 
elemtxt2 <- elem$getElementAttribute("innerHTML")[[1]] #get us the HTML
#elemxml <- htmlTreeParse(elemtxt, userInternalNodes = TRUE) #parse string into HTML tree to allow for querying with XPath
```
