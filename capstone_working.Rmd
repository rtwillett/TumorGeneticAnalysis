...
```{r}
#First attempt at a hierarchical analysis. This is going to have to do better. Right now it makes an enormous riot of all 801 tumors that is really hard to interpret.
#Promisingly, there do seem to be 5 large subdivisions of the dendrogram that hopefully will correspond to the tumor classifications upon further analysis.
d <- dist(as.matrix(cancer_data))
hc <- hclust(d)
plot(hc)
```
I am soft on the detail of how this works. Particularly, I have to make sure that the Class variable is not being included as this will bias the clustering in the direction expected (partitioning trees by tumor Class).
...
tuk_gene53
```
This is a test of the lm() function for **TP53** alone. 
```{r}
#Or should I use the lm function for ANOVA?

testing <- lm(gene_18530 ~ Class, data = cancer_join)
summary(testing)
```
